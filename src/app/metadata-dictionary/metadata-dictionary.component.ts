 ...
 <div class="panel-group" id="accordion5" *ngIf="isProgramInd">
   <div class="panel panel-default" *ngFor="let prog of programInd let in = first;">
    ...
    <div id="{{prog.object.id}}" class="panel-collapse collapse" [ngClass]="{ in: in }">
      ...
      <p>1.This <strong>{{prog.filterName}}</strong> filter will filter the events which become part of the evaluation / aggregation routine.</p>
      <p>2.This <strong>{{prog.object.expression}}</strong> expression will be evaluated per event.</p>
            ...
            self.get(programUrl)
              ...
              .subscribe(progInd=>{
                  ...
                  this.http.post(url,progInd.filter,{headers:headers})
                    ...
                    .subscribe(
                      data=>{
                        this.programInd.push({object:progInd,filterName:data.description})
